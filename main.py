...
def main():
    ...
    parser.add_argument("-i", metavar="<input file>", type=str,
                        help="Filename of the trained model. By default, it loads the last model version at the 'model/' directory.",
                        default="")
    ...
    parser.add_argument("--path", metavar="<pathname>", type=str, help='Path of the entire data (default: bds_stage/).',
                        default="bds_stage/")
    parser.add_argument("--threshold", metavar="float value", type=float,
                        help='Threshold value of the matching phase (default=0.7).', default=0.7)
    parser.add_argument("-o", metavar="<output path>", type=str,
                        help="Pathname to write the trained model. By default, it writes at the 'model/' directory.",
                        default="")
        ...
        if getattr(args, "c") is not None:
            ...
            if filename is not "":
                if os.path.exists(filename):
                    ...
                    print(
                        "Please, inform the properly pathname of the trained data: {} --db <database> -c <column> -i <inputfile>".format(
                            sys.argv[0]))
                    exit(2)
            else:
                path = "model/"
    ...
    # Trainning case: generate a trained model.
        ...
        if os.path.isdir(pathname):
            ...
            print("Please, inform the properly path of the data: python3.6 {} --train --path <pathname>".format(
                sys.argv[0]))
            exit(2)
